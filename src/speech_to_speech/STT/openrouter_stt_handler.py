...
class OpenRouterSTTHandler(BaseSTTHandler):
    """Cloud STT handler for OpenRouter transcription models.

    This currently transcribes finalized VAD turns only via
    ...
    def setup(
        ...
        if not self.api_key:
            raise ValueError("OpenRouter STT requires openrouter_stt_api_key or OPENROUTER_API_KEY.")
    ...
    def process(self, vad_audio: STTIn) -> Iterator[STTOut]:
        ...
        try:
            ...
            latency_s = perf_counter() - started_at
            logger.error(
                "OpenRouter STT failed turn=%s rev=%s model=%s after %.3fs: %s",
                vad_audio.turn_id,
        ...
        if not text:
            logger.debug(
                "OpenRouter STT returned no text for turn=%s rev=%s",
                vad_audio.turn_id,
                vad_audio.turn_revision,
            )
            return
        ...
        logger.info(
            "OpenRouter STT completed turn=%s rev=%s model=%s audio=%.3fs latency=%.3fs chars=%d cost=%s",
            vad_audio.turn_id,
            vad_audio.turn_revision,
            self.model_name,
